...
"""
  ...
  for v in CANNED_RESPONSES
    if key is v.key
      if v.url_regex.test url
        resp = v
        break
      else
        console.log "Canned response for " + key + " found, but url is invalid:" + url + ". Should be like " + v.url_regex 
  if not resp
    return null
  return [
    resp.statusCode
