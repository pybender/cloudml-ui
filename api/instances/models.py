...
class Cluster(BaseModel, db.Model):
    ...
    STATUSES = [STATUS_NEW, STATUS_STARTING, STATUS_RUNNING,
                STATUS_WAITING, STATUS_ERROR,
                STATUS_TERMINATED, STATUS_TERMINATING, STATUS_TERMINATED_WITH_ERRORS]

    TERMINATED_STATUSES = [STATUS_TERMINATED, STATUS_TERMINATING, STATUS_TERMINATED_WITH_ERRORS]
    ...
    def generate_port(self):
        ...
        exclude = set([cl[0] for cl in Cluster.query.with_entities(
            Cluster.port).filter(Cluster.status != Cluster.STATUS_TERMINATED,
